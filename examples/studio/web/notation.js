...
export function formulaText(spec,fields,parameters){
...
export function relationNotation(initial,fields,parameters,expr,parse){
  ...
  const write=el('button','Write a rule',{type:'button','data-rule-write':'','aria-pressed':'false'});
  ...
  input.oninput=()=>{generation++;feedback.textContent='Draft rule. Preview checks which items match.'};
  write.onclick=()=>{
    if(mode==='text'||pending)return;
    try{input.value=formulaText(editor.read(),fields,parameters);show('text');feedback.textContent='The same rule, written as text.';changed();input.focus({preventScroll:true});input.scrollIntoView({block:'center'})}
    catch(error){feedback.textContent=error.message}
  };
  controls.onclick=async()=>{
    if(mode==='controls'||pending)return;
    const token=++generation,text=input.value;pending=true;controls.disabled=write.disabled=true;changed();feedback.textContent='Reading the rule…';
    try{
      ...
      feedback.textContent='Rule translated to controls. Preview still checks its result.';
