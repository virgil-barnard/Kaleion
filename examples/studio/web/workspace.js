...
export function spatialWorkspace(host,{select,details,settings,options,combine,inspect,create,locked,status}){
  ...
  function syncTools(){
    ...
    help.textContent=tool==='move'?'Drag a name to move · drag the background to pan · right-click or hold for details.':tool==='connect'?'Drag a source name onto a destination to propose a combination. Dashed paths are reads; definitions remain fixed.':'Drag the canvas to orbit. XY / XZ / YZ and Rotate offer the same view changes without dragging.';
    list.hidden=tool!=='connect';
  }
  function presented(obj){
    if(!presentation||obj.name!==active)return obj;
    const rows=presentation.positions.map((position,i)=>{
      const original=obj.rows.find(r=>r.ref[1]===(presentation.after[i]||presentation.before[i]));
      const pair=presentation.labels?.[i],value=pair?(presentation.fraction===0?pair[0]:pair[1]):original?.fields.value;
      const fields={...original?.fields};delete fields.value;if(value!=null)fields.value=value;
      const membership=(presentation.fraction===0?presentation.matched_before:presentation.matched_after)?.[i];
      return {ref:original?.ref||[obj.capture,presentation.after[i]||presentation.before[i]],
        fields,position,match:membership??true,opacity:presentation.opacity[i],replay:true};
    });
    return {...obj,rows,placed:true,dimension:presentation.positions[0]?.length||obj.dimension||2};
  }
  ...
  function fitBounds(all){
    ...
    const rect=canvas.getBoundingClientRect(),panel=document.getElementById('panel'),dock=!panel.hidden?panel.getBoundingClientRect():null;
    const right=dock&&dock.x>rect.x+rect.width/2?rect.right-dock.x+20:20;
    const bottom=dock&&dock.x<=rect.x+rect.width/2?Math.max(80,rect.bottom-dock.y+20):90;
    const width=Math.max(180,rect.width-right-30),height=Math.max(120,rect.height-bottom-85);
    const left=Math.min(...all.map(b=>b.left))-20,end=Math.max(...all.map(b=>b.right))+20,top=Math.min(...all.map(b=>b.top))-20,low=Math.max(...all.map(b=>b.bottom))+20;
    const unit=Math.max(.55,(end-left)/width,(low-top)/height);
    view={...view,x:(left+end-width*unit)/2-30*unit,y:(top+low-height*unit)/2-85*unit,w:rect.width*unit,h:rect.height*unit};draw();
  }
  function fit(){cancel();draw();fitBounds([...bounds.values()])}
  function centerSelected(){cancel();const b=bounds.get(active);if(b)fitBounds([b])}
  function zoom(factor){cancel();const w=Math.min(1e9,Math.max(80,view.w/factor)),h=w*view.h/view.w;view={...view,x:view.x+(view.w-w)/2,y:view.y+(view.h-h)/2,w,h};draw()}
  ...
  function draw(){
    ...
    const pixel=view.w/Math.max(320,canvas.getBoundingClientRect().width);
    ...
    for(const obj of [...state.objects].sort((a,b)=>(b.name===active)-(a.name===active))){
      const settings=drawingSettings(obj),model=models.get(obj.name),rows=presented(obj).rows;let count=0;
      model.positions.forEach((p,i)=>{if(!model.visible(p)||budget<=0)return;budget--;count++;const world=plus(p,settings.pose),screen=project.point(world);drawn.push({name:obj.name,row:rows[i],local:p,world,screen,settings,model})});counts.set(obj.name,count);
    }
    for(const obj of state.objects){
      ...
      const meta=obj.status==='failed'?'Evaluation failed':!m.valid?'Geometry unavailable':`${obj.preview?'Preview · ':''}${total} items${s.slice?` · ${visible} in slice`:''}${counts.get(obj.name)<visible?` · ${counts.get(obj.name)} drawn`:''}`;
      const labelWidth=Math.max(130,Math.min(320,Math.max(obj.name.length*8,meta.length*5.5)+22))*pixel;
      const bound={left:b.left+offset[0]-14,right:Math.max(b.right+14,b.left-10+labelWidth)+offset[0],top:b.top+offset[1]-53*pixel,bottom:b.bottom+offset[1]+14};bounds.set(obj.name,bound);
      ...
      frames.append(frame);
      const card=svgNode('g',{'data-workspace-object':obj.name,class:`workspace-object ${obj.name===active?'selected':''}`,transform:`translate(${offset[0]},${offset[1]})`,role:'button',tabindex:'0','aria-label':`${obj.name}. Drag name to move view. Space to select; Enter or Shift F10 for details.`,'aria-pressed':String(obj.name===active)});
      card.append(svgNode('rect',{x:b.left-10,y:b.top-52*pixel,width:labelWidth,height:40*pixel,rx:7*pixel,class:'scene-handle'}),svgNode('text',{x:b.left,y:b.top-34*pixel,class:'workspace-name',style:`font-size:${14*pixel}px`},obj.name.length>30?obj.name.slice(0,28)+'…':obj.name));
      card.append(svgNode('text',{x:b.left,y:b.top-19*pixel,class:'workspace-meta',style:`font-size:${11*pixel}px`},meta),svgNode('title',{},`${obj.name}. ${s.chart==='logical'?'Logical chart: '+m.axes.join(', '):'Declared placement'}. View offset ${s.pose.join(', ')}.`));
  ...
  function syncSettings(){
    ...
    const m=models.get(active);sliceAxis.replaceChildren(el('option','All items',{value:''}),...m.axes.map(a=>el('option',`Slice ${a}`,{value:a})));sliceAxis.value=s.slice?.axis||'';
    ...
    note.textContent=(s.chart==='logical'?`Logical chart: ${m.axes.join(', ')}. `:`Declared ${obj.dimension}D placement. `)+(m.dimension===3?'Cells are cube glyphs. ':'Cells are unit glyphs. ')+(obj.axes?.length>m.axes.length&&s.chart==='logical'?'Other axes are projected out; items may overlap. ':'')+'Slices filter this view only; no new relation is created.';
  }
  function syncSelection(project){
    const obj=state.objects.find(o=>o.name===active),m=models.get(active);selection.replaceChildren(el('option','Choose an item',{value:''}));
    ...
    for(const {r,i,depth} of items)selection.append(el('option',`${r.fields.index}${r.fields.value===undefined?' · tuple only':` · value ${r.fields.value}`}${r.match?'':' · outside relation'}${m.positions[i]&&!m.visible(m.positions[i])?' · outside slice':''}`,{value:refKey(r.ref)}));
    ...
    selectionNote.textContent=row?`Selected item ${row.r.fields.index}${row.r.fields.value===undefined?' · tuple only':` · value ${row.r.fields.value}`}${p?' · '+m.axes.map((a,i)=>`${a}=${p[i]}`).join(', '):''}${p&&!m.visible(p)?' · hidden by the view slice':''}`:'Tap a mark or use this list to reach overlapping and hidden items.';
  }
  ...
  canvas.onpointerdown=e=>{
    if(presentation||e.button!==0||locked())return;e.preventDefault();pointers.add(e.pointerId);canvas.setPointerCapture(e.pointerId);if(pointers.size!==1){cancel();return}
    ...
    if(e.pointerType!=='mouse')drag.timer=setTimeout(()=>{cancel();holdClick=true;options(name||null)},480);
  };
  ...
  canvas.oncontextmenu=e=>{e.preventDefault();cancel();if(!locked())options(nameAt(e)||null)};
  canvas.ondblclick=e=>{if(locked())return;const name=nameAt(e);if(name)details(name);else create()};
  canvas.onkeydown=e=>{
    const name=nameAt(e)||active;
    if(e.key==='Escape'){e.preventDefault();cancel();return}if(locked())return;
    if(!name){if(e.key==='ContextMenu'||(e.shiftKey&&e.key==='F10')){e.preventDefault();create()}return}
    if(e.key===' '){e.preventDefault();select(name)}if(e.key==='Enter'){e.preventDefault();details(name)}
