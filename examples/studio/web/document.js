...
// would round exact integers. This envelope owns presentation choices only.
...
export function validateScene(scene){
  if(!keys(scene,['version','camera','objects','selected','tool'])||scene.version!==1)throw Error('Unsupported canvas view version.');
  ...
  for(const o of scene.objects){
    if(!keys(o,['name','pose','marks','chart','axes','slice'])||typeof o.name!=='string'||!o.name||o.name.length>80||names.has(o.name)||!vector(o.pose))throw Error('Invalid or repeated saved object view.');
    names.add(o.name);
...
export function canvasDocument(workspace,scene){
  ...
  validateScene(scene);
  return JSON.stringify({format:'kaleion-studio',version:1,workspace,scene},null,2);
}
export function readDocument(text){
  const data=JSON.parse(text);
  if(data?.format==='kaleion-studio'){
    if(!keys(data,['format','version','workspace','scene'])||data.version!==1||typeof data.workspace!=='string')throw Error('Unsupported canvas document version.');
