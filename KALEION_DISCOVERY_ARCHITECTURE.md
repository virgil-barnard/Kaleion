...
The current suite includes 74 automated tests. The quotient/remainder fixture executes all 144 pairs 2≤a,b≤13, including non-coprime repeated/omitted source rows. Additional checks cover all three W4 extension matrices, Lookup/Gather commutation under a fixed map, a 4³ lattice plane with 16 matches, arbitrary scattered 3D points driven by keyed counts, count-driven values and constructor parameters, source-order independence, retained zero groups and empty fibers, contributor provenance, and exact integer arithmetic beyond 64-bit range. The notebook additions cover reciprocal floor sums, the three-incidence box, measured motion, finite Radon reconstruction, Young layers, additive energy, and Ehrhart counts; their evidence is in [notebooks/VALIDATION.md](notebooks/VALIDATION.md).
...
The first core refactor repairs composition after `Incidence.with_params`, exposes `Incidence.universe`, separates field interpretation from graph execution, prepares motion correspondence once per captured root, and replaces the quadratic contributor loop with ordered buckets. The next pass establishes ownership for snapshot buffers and containers, reuses unchanged validated data, normalizes NumPy integer inputs before arithmetic, and centralizes key/group/address rules in `indexing.py`. Tests preserve scopes, operation-specific identity and placement policies, exact contributors, captured reversal, and JSON reopening. The [refinement plan](docs/CORE_REFINEMENT_PLAN.md) records remaining session/handler separation and the distinction between definition dependencies and per-target driver alignment evidence, which a general explanation view still needs.
