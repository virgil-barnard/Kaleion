...
(async()=>{
 ...
 try{
  ...
  assert.equal(await page.locator('#arrange').isVisible(),true);assert.equal(await page.locator('#advanced-actions [data-action="place"]').count(),0);await page.screenshot({path:path.join(output,'arrange-move.png')});
  const unedited=await exported();await page.locator('#arrange').click();await preview();assert.match(await page.locator('#activity').innerText(),/record the change as motion/);assert.equal(await page.locator('#scene-chart').inputValue(),'placement');await page.locator('#cancel').click();await idle();assert.equal(await page.locator('#scene-chart').inputValue(),'logical');assert.equal(await exported(),unedited);
  await details('Markers');await page.locator('#arrange').click();await apply();assert.equal(await page.locator('#scene-chart').inputValue(),'placement');await load('00_first_motion');
